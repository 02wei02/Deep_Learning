...
class SincNet(nn.Module):
    def __init__(self, cfg=None):
        ...
        if cfg is None:
            cfg = [40, 'P', 256, 'P', 256, 'P', 256, 'P', 256, 'P', 160, 'P']
        
        self.sincconv = _Layer(
            in_channels=1, out_channels=cfg[0], kernel_size=101, stride=1, max_pool_size=2, use_sinc=True)

        self.features = nn.ModuleList()     

        self.features.append(_Layer(in_channels=cfg[0], out_channels=cfg[2],
                                    kernel_size=25, stride=2, max_pool_size=2, use_sinc=False))

        self.features.append(_Layer(in_channels=cfg[2], out_channels=cfg[4],
                                    kernel_size=9, stride=1, max_pool_size=2, use_sinc=False))

        self.features.append(_Layer(in_channels=cfg[4], out_channels=cfg[6],
                                    kernel_size=15, stride=1, max_pool_size=2, use_sinc=False))

        self.features.append(_Layer(in_channels=cfg[6], out_channels=cfg[8],
                                    kernel_size=9, stride=1, max_pool_size=2, use_sinc=False))

        self.features.append(_Layer(in_channels=cfg[8], out_channels=cfg[10],
                                    kernel_size=9, stride=1, max_pool_size=2, use_sinc=False))

        self.gap = nn.AdaptiveAvgPool1d(1)
        
        self.fc = nn.Linear(cfg[10], 10)
